...
def draw_afd(app: "App", win) -> None:
    ...
    afd = app.afd
    if not afd or not afd.get("text"):
        safe_addstr(
            win, 0, 0,
            "No forecast discussion available yet.",
            curses.color_pair(3) | curses.A_BOLD,
        )
        win.noutrefresh()
        return

    office = afd.get("office") or "—"
    issued_dt = parse_iso(afd.get("issuance_time"))
    issued_s = fmt_time(issued_dt, app.use_24h, True) if issued_dt else "—"
    header = f"Area Forecast Discussion — {office}   Issued: {issued_s}"
    safe_addstr(win, 0, 0, header[: cols - 1], curses.color_pair(15) | curses.A_BOLD)

    lines = render_text_product(afd["text"], max(1, cols - 1))
    total = len(lines)
    view_rows = rows - 3

    app.afd_scroll = clamp(app.afd_scroll, 0, max(0, total - max(1, view_rows)))

    y = 2
    for line, is_title in lines[app.afd_scroll:]:
        if y >= rows - 1:
            break
